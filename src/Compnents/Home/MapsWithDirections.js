...
export default function Map(props) {
  ...
  const [cars, setCars] = useState(
    props.source.length > 1 ? props.source : [11.0018115, 76.9628425]
  );
    ...
    window.addEventListener('keydown', (event) => {
      ...
      if (event.keyCode === 88) {
        ...
        const clear = setInterval(() => {
          setFlag(false);
          if (routes && routes.one && routes.one.routeLegs) {
            if (i < routes.one.routeLegs[0].itineraryItems.length) {
              setCars([
                routes.one.routeLegs[0].itineraryItems[i].maneuverPoint
                  .coordinates[0],
                routes.one.routeLegs[0].itineraryItems[i].maneuverPoint
                  .coordinates[1],
              ]);
  ...
  function getRoute() {
    ...
    for (let i in props.dest) {
      ...
      if (i == 0) start = items[Object.keys(items)];
      else end = items[Object.keys(items)];
    }
    // console.log(start,end)
    Axios.get(
      `${process.env.REACT_APP_BINGMAPS_ROUTE_API_1}${start}${process.env.REACT_APP_BINGMAPS_ROUTE_API_2}${end}${process.env.REACT_APP_BINGMAPS_ROUTE_API_3}${process.env.REACT_APP_BINGMAPS_KEY}`
    )
      .then((res) => {
