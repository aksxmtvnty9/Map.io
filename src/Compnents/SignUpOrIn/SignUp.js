...
export default function SignUp() {
  ...
  useEffect(() => {
    document.title = 'Sign Up | Welcome to Map.io';
    toast.configure({
      autoClose: 2000,
      draggable: false,
  ...
  function checkAuthDetails(event) {
    ...
    if (authDetails) {
      ...
      if (
        event.target.value === userDetails[event.target.name] &&
        userDetails[event.target.name].length > 0
      ) {
        ...
        if (event.target.name === 'name') {
          setUserDetails({
            ...userDetails,
            userName: userDetails.name.toLowerCase().replace(/\s/g, ''),
          });
        } else {
  ...
  function pushUsers(event) {
    ...
    if (flag) {
      if (users) {
        ...
        alertSuccess(
          'Hurray..Log in and have our features at your finger tips'
        );
  ...
  function spanAlert() {
    if (notify.substr(0, 1) === 't') {
      return (
        <span className="alert-span success">
          What a spectacular {notify.substr(1)}!!
        </span>
      );
    } else if (notify.substr(0, 1) === 'f') {
      return (
        <span className="alert-span danger">
          {notify.substr(1)} already exists, please choose a unique one.
        </span>
      );
    }
  }
